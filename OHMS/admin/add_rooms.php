...
if(isset($add))
{
	$sql=mysqli_query($con,"select * from rooms where room_no='$rno' and hotel_id in( select hotel_id from hotels where hotel_name='$hna')");
	...
	{	
	$img=$_FILES['img']['name'];
	$sql=mysqli_query($con,"select * from hotels where hotel_name='$hna'");
	$res=mysqli_fetch_assoc($sql);
	$res1=$res['hotel_id'];
	mysqli_query($con,"insert into rooms values('','$rno','$type','$price','$details','$img','$res1')");	
...
<table class="table table-bordered">
	<tr>	
		<th>Hotel Name</th>
		<td><input type="text" name="hna"  class="form-control"/>
		</td>
	</tr>
	<tr>	
		<th>Room No</th>
